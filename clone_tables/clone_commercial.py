"""
Script para clonar todas as tabelas do schema 'commercial' do PostgreSQL PRD AWS
para o schema 'gmcommercial' do PostgreSQL HML Azure
"""
...
# Configurar logging - arquivo TXT no mesmo diretório
log_file_path = os.path.join(os.path.dirname(__file__), 'clone_commercial.txt')
logging.basicConfig(
    level=logging.INFO,
...
def get_all_tables_from_commercial_schema():
    """Lista todas as tabelas do schema 'commercial' no PostgreSQL PRD AWS"""
    print("\n" + "="*80)
    print("ETAPA 1: Listando tabelas do schema 'commercial' no PostgreSQL PRD AWS")
    print("="*80)
    
    config = DatabaseConnection.POSTGRESQL_PRD_CONFIG
    print(f"PRD - Database: {config['database']}")
    print(f"PRD - Schema: commercial")
    ...
    try:
        ...
        # Buscar todas as tabelas do schema 'commercial'
        query = """
        SELECT 
            table_name,
            table_type
        ...
        print(f"\nOK - Encontradas {len(table_names)} tabelas no schema 'commercial' do database '{current_db}':")
        for i, table_name in enumerate(table_names, 1):
            print(f"  {i}. {table_name}")
        
        logger.info(f"Database PRD: {current_db}, Schema: commercial, Tabelas encontradas: {len(table_names)}")
...
def get_table_ddl_pg_dump(table_name):
    ...
    try:
        ...
        # Substituir schema 'commercial' por 'gmcommercial' (evitar duplicação)
        # Primeiro substituir referências completas, depois genéricas
        ddl = re.sub(r'\bcommercial\.', 'gmcommercial.', ddl)
        ddl = ddl.replace('CREATE TABLE gmcommercial.', 'CREATE TABLE gmcommercial.')
        ddl = ddl.replace('TABLE gmcommercial.', 'TABLE gmcommercial.')
        # Garantir que não tenha gmgmcommercial
        ddl = re.sub(r'gmgmcommercial', 'gmcommercial', ddl)
...
def get_table_ddl(table_name):
    """Obtém o DDL (CREATE TABLE) de uma tabela do schema 'commercial'"""
    ...
    try:
        ...
        cursor.execute("""
            ...
            WHERE tc.table_schema = 'commercial'
            AND tc.table_name = %s
        ...
        # Construir DDL
        ddl_parts = [f"CREATE TABLE gmcommercial.{table_name} ("]
        ...
        for index in indexes:
            ...
            index_def = index[1]
            # Substituir referências ao schema commercial
            index_def = re.sub(r'\bcommercial\.', 'gmcommercial.', index_def)
            # Garantir que não tenha gmgmcommercial
            index_def = re.sub(r'gmgmcommercial', 'gmcommercial', index_def)
...
def create_table_in_hml(table_name, ddl):
    """Cria uma tabela no PostgreSQL HML Azure usando o DDL fornecido"""
    config = DatabaseConnection.POSTGRESQL_HML_DESTINO_CONFIG
    ...
    try:
        ...
        if current_schema != 'gmcommercial':
            logger.warning(f"Schema atual é '{current_schema}', configurando para 'gmcommercial'...")
            cursor.execute("SET search_path TO gmcommercial, public;")
            conn.commit()
        ...
        if table_exists:
            print(f"  AVISO - Tabela {table_name} já existe no schema 'gmcommercial'. Pulando...")
            logger.warning(f"Tabela {table_name} já existe no HML (database: {current_db}, schema: gmcommercial)")
            cursor.close()
            conn.close()
            return False
        ...
        if created:
            logger.info(f"Tabela {table_name} criada com sucesso no HML (database: {current_db}, schema: gmcommercial)")
...
def validate_tables_created():
    ...
    if not missing_tables and not extra_tables:
        ...
        logger.info(f"Validação OK - PRD: {prd_db}.commercial ({len(prd_tables)} tabelas) = HML: {hml_db}.gmcommercial ({len(hml_tables)} tabelas)")
        return True
    else:
        logger.warning(f"Validação com diferenças - PRD: {prd_db}.commercial ({len(prd_tables)} tabelas) vs HML: {hml_db}.gmcommercial ({len(hml_tables)} tabelas)")
        return False
...
def main():
    ...
    print("\n" + "="*80)
    print("CLONAGEM DE TABELAS: PRD AWS (commercial) -> HML Azure (gmcommercial)")
    print("="*80)
    ...
    try:
        ...
        tables = get_all_tables_from_commercial_schema()
        
        if not tables:
            print("\nAVISO - Nenhuma tabela encontrada no schema 'commercial' do PRD")
            return
